...
Requirements
-----
* Note: Tested on macOS Sequoia and Ubuntu 24.04 LTS.
...
* Build the software with the commands below. 
    ...
    Note: When running `make clean`, you may see a message like "make: *** No rule to make target". But, you can ignore this message and move to `qmake visuco.pro -r -spec linux-g++-64` (make clean is to avoid the reuse of the existing outcomes from qmake/make).

* Launch "visuco" application generated by the above build process in the same directory. 
    - Bug Note: When tested on Ubuntu 24.04 running in VMware Fusion, the lasso interaction is not drawn correctly. This issue only affects the visual effect---the selection itself still works based on mouseover positions. Contributions to fix this bug are welcome.
