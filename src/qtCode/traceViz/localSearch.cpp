    ...
    while (improvement  &&  evaluations < 100)
    {
        ...
        for (int i = 0; i < dimension_ ; ++i)
        {
            ...
            if (dlb_[i] == false)
            {
                ...
                {
                    // Get the moving cost
                    float movingCost = moveCost(solution_, i , j);
...
float LocalSearch::calculateCost(vector<int> &solution)
{
    float cost = 0;
    for (int i = 0; i < dimension_; ++i)
    {
...
float LocalSearch::moveCost(vector<int> &oldSolution, int r, int s)
{
    float cost = 0;
