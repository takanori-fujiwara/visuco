...
Memetic::Memetic(const Input &input, int maxEvaluations, int populationSize, double probabilityLS, bool initRandom)
      ...
      populationSize_ (populationSize), evaluations_(0), probabilityLS_(probabilityLS)
{
    ...
    /// Edited here
    // non-randomized intial inut
    solution_.clear();
    for (int i = 0 ; i < dimension_; ++i) {
        solution_.push_back(i);
    }
    // original cost
    costWithOriginalOrder = calculateCost(solution_);
    cout << "Original Cost: " << costWithOriginalOrder << endl;
    ...
    // in case memetic does not find good result
    if (cost_ > costWithOriginalOrder) {
        cost_ = costWithOriginalOrder;
        solution_.clear();
        for (int i = 0 ; i < dimension_; ++i) {
            solution_.push_back(i);
        }
...
void Memetic::execute()
{
    ...
    cout << "Result Cost: " << cost_ << endl;
    cout << "Improve Rate: " << (1.0 - cost_ / getCostWithOriginalOrder()) * 100 << "%" << endl;
}
...
float Memetic::calculateCost(vector<int> &solution)
{
    float cost = 0;
    for (int i = 0; i < dimension_; ++i)
    {
...
float Memetic::getCost()
{
    return cost_;
}

float Memetic::getCostWithOriginalOrder()
{
    return costWithOriginalOrder;
}
